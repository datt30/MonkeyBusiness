...
function initialize(){
  ...
  formRegister_contactos.addEventListener("submit",send_noticia_toFirebase, false);

  refUrbanizacion = firebase.database().ref().child("unidad x");
  refUrbanizacion_noticias= refUrbanizacion.child("noticias");
}

function send_noticia_toFirebase(event){
  event.preventDefault();

  //GUARDAMOS LA IMAGEN
  var file = event.target.img.files[0];
  var filename = file.name;
  storageRef = firebase.storage().ref('/unidadx/noticias/'+ filename);
